# Migrating Coms Coupler onto `wollerp/auth-client`

> **This is a migration runbook for one existing product, not the integration
> guide.** Standing up a new product? Read [`INTEGRATION.md`](INTEGRATION.md)
> instead — it is shorter, and none of it is about Passport, `auth_db`, or 88
> files that already reference a `User` model.
>
> Read this one only if the product you are working on **already has its own
> authentication** and has to keep serving traffic while it moves. Roughly half
> of what follows — the soak period, dual-accept, the nine legacy `'id'` call
> sites, the phase-5 deletion list — exists solely because Coms Coupler predates
> the auth server. A greenfield product has none of those problems and must not
> inherit their solutions; in particular step 4's `auth:wollerp` is the right
> call **here** and the wrong default **there**, for a reason spelled out in
> that step.
...
**No hand-edits are needed any more.** Two things that used to be manual are not:

**(a) The `id` alias is in the stub.** `users_mirror` ships with
`$table->unsignedInteger('id')->virtualAs('auth_user_id')` and its index, guarded
on the driver. See step 5 for what it is for — the short version is that CC has
nine expressions querying the literal column `id` and this keeps every one of
them working untouched. Confirm it came through, because if it did not, step 5's
"zero application edits" claim is false:

```bash
grep -n "virtualAs" database/migrations/*_create_users_mirror_table.php
```

**(b) Both tables belong on `product_db`.** The stubs read
`config('wollerp-auth.database.connection')`, call `Schema::connection(...)`, and
override `getConnection()` so the `migrations` ledger row lands on the same
connection. So you do **not** edit the migrations for this either — you set
`WOLLERP_AUTH_DB_CONNECTION=product_db` in step 3. Confirm before running:
...
after a package upgrade will offer to overwrite them. Since there is nothing to
re-apply, `vendor:publish --force` is now safe to run.
...
What `auth:wollerp` costs: Laravel's own `Authenticate` middleware reaches the
guard through `Guard::check()`, and `TokenGuard::user()` correctly swallows
`WollerpAuthException` and returns null, as the `Guard` contract requires. So
**every** failure arrives at Laravel as "not authenticated" and Laravel answers
401 — including a JWKS outage, which CONTRACT §3 says is a **503**. That breaks
§8 checks 9 and 10 below, which this very document says must be 503, and a 401
there sends users back through login for a fault on our side.

So if CC keeps `auth:wollerp` — and it should, because `ApiResponser` is its
convention and seven route groups already read `auth:api` — it **must** map the
exception family in `bootstrap/app.php`:

```php
->withExceptions(function (Exceptions $exceptions): void {
    $exceptions->render(function (WollerpAuthException $e, Request $request) {
        // 401 for a bad token; 503 for jwks_unavailable / jwks_unusable.
        return response()->json([
            'success' => false,
            'error_code' => 'AUTH_'.strtoupper($e->reason()),
...
That render hook is **not optional** with `auth:wollerp`. Without it, check 10 in
§8 cannot pass, and the first Auth incident looks to CC like a wave of expired
sessions rather than an availability event. Verify it with check 10 *before*
believing the route swap is done.

The alternative is the `wollerp.auth` alias, which does the 401/503 split itself
and needs no handler work — at the cost of the package's response envelope
instead of CC's.
...
**The generated column ships in the stub**, so this is handled as long as step
2(b)'s `grep` found it. Zero application edits; all nine keep working;
`keyBy('id')` and `$user->id` work because `id` is a real selected column.

The alternative — editing the nine sites to `whereIn((new User)->getKeyName(), …)`
plus a `getIdAttribute()` accessor for the `keyBy('id')` and `formatUserData()`
paths — is a smaller schema and a much larger diff, and it leaves a trap for the
next person who writes `where('id', …)` out of habit. It is documented here only
...
Route::post('/api/v1/internal/revoke', function (Request $request, DenylistChecker $denylist) {
    ...
    // The payload named neither a sid nor a jti, so nothing was revoked. Say so
    // — a 204 would tell the auth server a revocation it never performed had
    // succeeded, and it would stop retrying.
    if (! $written) {
        return response()->json([
            'success' => false,
...
`revoke()` takes `mixed` and normalises, so forwarding `$request->input(...)`
straight in is safe under CC's `declare(strict_types=1)`: `{"sid": 12345}` is
matched on as `"12345"` rather than raising a TypeError, and a non-scalar becomes
null and reports `false`. A 500 on a service-plane call is the worst of both
outcomes — the sender treats 5xx as retryable and re-delivers a payload that will
fail identically six times.
